...
### 1. Get the ICD-10-CM code set

This step is required and can't be skipped — the code descriptions aren't in the repo. CMS publishes ICD-10-CM for free, but I don't redistribute it here (it's ~74,000 rows, it's versioned by fiscal year, and you should pull the edition you actually want to code against).

1. Go to <https://www.cms.gov/medicare/coding-billing/icd-10-codes>.
2. Download the current **"ICD-10-CM Code Descriptions in Tabular Order"** zip.
3. Unzip it and find `icd10cm_codes_YYYY.txt` (a two-column file: code, then description).
4. Copy it to `data/icd10cm.txt` in this repo.

```bash
# from the repo root, after unzipping
cp ~/Downloads/icd10cm_codes_2025.txt data/icd10cm.txt
```

Quick check that it landed correctly — you should see ~74,000 lines starting with `A00 Cholera due to Vibrio cholerae 01, biovar cholerae`:

```bash
wc -l data/icd10cm.txt && head -1 data/icd10cm.txt
```

CPT needs no download: the repo ships a 37-code demo subset (`backend/src/data/load_cpt_demo.py`), because the full CPT set is AMA-copyrighted and can't be redistributed.
...
# Build the BM25 and Chroma indices (needs data/icd10cm.txt from step 1).
# Takes a few minutes the first time — it embeds ~74k descriptions on CPU.
...
If you skipped step 1, `build_indices` exits with `ERROR: .../data/icd10cm.txt not found`, and every `POST /code` request returns **503 Pipeline not initialized**. That's the expected failure, not a bug — go back and fetch the file.
...
The LLM call is mocked, so the suite is fast and costs nothing. It covers the negation detector and the three reranker safety guards: that a hallucinated code outside the candidate list gets dropped, that an LLM outage falls back to retrieval order with `needs_human_review` set, and the happy path end to end. Note that the tests still load spaCy for real, so `en_core_web_sm` must be installed.
